...
export interface IUserScoreRepository extends IBaseRepository<IUserScore> { 
  findOneAndUpsert$(leaderboardId: string, userId: string, fixtureId: string, predictionId: string, 
    predictionPoints: ScorePoints, hasJoker: boolean): Observable<IUserScore>;
...
export class UserScoreRepository extends BaseRepository<IUserScore> implements IUserScoreRepository {
  ...
  findOneAndUpsert$(leaderboardId: string, userId: string, fixtureId: string, predictionId: string, 
    predictionPoints: ScorePoints, hasJoker: boolean) {
    let { points, APoints, BPoints, MatchOutcomePoints, TeamScorePlusPoints,
      ExactScorePoints, GoalDifferencePoints, TeamScoreMinusPoints } = predictionPoints;
    ...
    return this.findOne$({ leaderboard: leaderboardId, user: userId })
      .flatMap(standing => {
        if(standing == null) {
          ...
          if(fixtureExists) { return Observable.of(standing) }      
